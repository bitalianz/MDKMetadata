...
import {
	ChecklistLibrary as libChecklist
} from '../../Checklists/ChecklistLibrary';
import Loc from '../../vanti/clLocation';

import CompleteCustomPart from '../../vanti/OnCompleteOrderPessedZPart';
...
export default class {
	...
	static startOperation(context) {
		...
		return context.executeAction('/SAPAssetManager/Actions/WorkOrders/MobileStatus/OperationStartUpdate.action').then(function () {
				context.setToolbarItemCaption('IssuePartTbI', "Continuar");
				...
				context.binding.oPagesClear = oPages.conf;
	...
	static completeOperation(context) {
		...
		Loc.getCurrentLocation(context, function (oLoc) {
			...
			oBinding.ubicacionFinal = sLat + "," + sLong;
			...
			if (results === true) {
				return this.showOperationCompleteWarningMessage(context).then(
					doMarkComplete => {
						...
						CompleteCustomPart(context, function (sRet) {
							if (sRet) {
								...
								return Promise.all(promises).then(() => {
									...
									return pageContext.executeAction(
										'/SAPAssetManager/Actions/WorkOrders/MobileStatus/OperationMobileStatusFailureMessage.action');
								});
							} else {
								return Promise.resolve(true);
							}
	...
	static operationStatusPopoverMenu(context) {
		...
		var started = libCommon.getAppParam(context, 'MOBILESTATUS', context.getGlobalDefinition(
			'/SAPAssetManager/Globals/MobileStatus/ParameterNames/StartParameterName.global').getValue());
		/*
		...
		if (libMobile.isHeaderStatusChangeable(context)) {
			return libMobile.mobileStatus(context, context.binding).then(() => {
				return libWOMobile.getWorkOrderMobileStatus(context).then(headerMobileStatus => {
					...
					context.dismissActivityIndicator();
					return libCommon.getAppParam(context, 'MOBILESTATUS', context.getGlobalDefinition(
						'/SAPAssetManager/Globals/MobileStatus/ParameterNames/CompleteParameterName.global').getValue());
				});
			});
		}
		/*
							...
							results => {
								if (results) {
									...
									if (item) {
										...
										var oprHold = libCommon.getAppParam(context, 'MOBILESTATUS', context.getGlobalDefinition(
											'/SAPAssetManager/Globals/MobileStatus/ParameterNames/HoldParameterName.global').getValue());
										var oprReceived = libCommon.getAppParam(context, 'MOBILESTATUS', context.getGlobalDefinition(
											'/SAPAssetManager/Globals/MobileStatus/ParameterNames/ReceivedParameterName.global').getValue());
	...
	static operationRollUpMobileStatus(context, entitySet) {
		...
		if (!isLocal) {
			...
			var started = libCommon.getAppParam(context, 'MOBILESTATUS', context.getGlobalDefinition(
				'/SAPAssetManager/Globals/MobileStatus/ParameterNames/StartParameterName.global').getValue());
			var hold = libCommon.getAppParam(context, 'MOBILESTATUS', context.getGlobalDefinition(
				'/SAPAssetManager/Globals/MobileStatus/ParameterNames/HoldParameterName.global').getValue());
			var complete = libCommon.getAppParam(context, 'MOBILESTATUS', context.getGlobalDefinition(
				'/SAPAssetManager/Globals/MobileStatus/ParameterNames/CompleteParameterName.global').getValue());
			return context.read('/SAPAssetManager/Services/AssetManager.service', entitySet, ['OperationNo', 'OrderId', 'ObjectKey'],
					"$filter=(OrderId eq '" + orderID + "')&$orderby=OrderId")
