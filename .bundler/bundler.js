  ...
  _buildAppIndexContent(aBundleFiles) {
    ...
    return instance._getIntegrationTargetSourceMappings(instance.clientDefinitionsPath, aMappings).then(() => {
      ...
      aMappings.forEach(oMapping => {
        ...
        sourceTargetMap[sTarget] = sSource;
      });

      if (Object.keys(sourceTargetMap).length > 0) {
        aModifiedExports = aExports.map(oTargetItem => {
          let sSource = sourceTargetMap[oTargetItem.name];
          if (sSource) {
            while(sourceTargetMap[sSource]) {
              sSource = sourceTargetMap[sSource];
            }
            let oTargetExport = aExports.find(oSourceItem => {
              if (oSourceItem.name === sSource) {
                return true;
              } 
            });
            if (oTargetExport) {
              oTargetItem.path = oTargetExport.path;
            }
          }
          return oTargetItem;
        });
      ...
      let compiler = webpack({
        ...
        module: {
          rules: [
            ...
            {
              test: /\.ts$/,
              include: path.resolve(path.join(instance.buildOutputPath, instance.sBaseApplicationName), 'Extensions'),
              loader: 'ts-loader',
              options: {
                transpileOnly: true
              }
